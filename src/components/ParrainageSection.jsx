...
export default function ParrainageSection({ theme, user, setView, setModal, notify, t, boutiques, ateliers, restos, view, inputStyle, cardStyle, referralStats, supabase }) {
  return (
    <>
{view==="parrainage"&&(
  <div style={{ width:"100%",maxWidth:700,margin:"0 auto",padding:"48px 40px",animation:"fadeIn 0.4s ease" }}>
    ...
    {user ? (
      <div style={{ ...cardStyle,borderRadius:20,padding:32 }}>
        ...
        {referralStats.refCode && <p style={{ color:"#FFD700",fontWeight:800,fontSize:18,marginBottom:8,letterSpacing:2 }}>🎫 Code : <span style={{ background:"rgba(255,215,0,0.1)",border:"1px solid rgba(255,215,0,0.3)",borderRadius:8,padding:"4px 12px" }}>{referralStats.refCode}</span></p>}
        <div style={{ display:"flex",gap:8,marginBottom:20 }}>
          <input readOnly value={`https://marcheduroi.com?ref=${referralStats.refCode||user.id}`} style={{ ...inputStyle,flex:1,background:theme.bg }} onClick={e=>e.target.select()}/>
          <button onClick={()=>{ navigator.clipboard.writeText(`https://marcheduroi.com?ref=${referralStats.refCode||user.id}`); notify("Lien copié ! 📋"); }} style={{ background:"linear-gradient(135deg,#6C63FF,#8B84FF)",border:"none",color:"#fff",padding:"12px 16px",borderRadius:10,fontWeight:700,cursor:"pointer" }}>Copier</button>
        </div>
        <a href={`https://wa.me/?text=${encodeURIComponent("Rejoins-moi sur MarcheduRoi, la plateforme multipolaire pour booster ton business !\nInscris-toi gratuitement ici : https://marcheduroi.com?ref="+(referralStats.refCode||user.id))}`} target="_blank" rel="noopener noreferrer" style={{ textDecoration:"none",display:"block" }}>
